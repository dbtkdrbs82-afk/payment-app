...
<div id="business-info-box">
  ...
  <label id="corporate-number-label" style="display:none;">
    법인번호 *
  </label>
...
const applyAgencySelect =
  document.querySelector<HTMLSelectElement>('#apply-agency-name')

if (applyAgencySelect) {
  const { data: agencyUsers, error: agencyError } = await supabase
    .from('admin_users')
    .select('id, admin_name, role')
    .in('role', ['BRANCH', 'AGENCY', 'MANAGER'])
    .eq('status', '사용중')
    .order('id', { ascending: true })

    if (agencyError) {
      alert('소속 목록 조회 실패: ' + agencyError.message)
    } else {
      applyAgencySelect.innerHTML =
        ...
        (agencyUsers || []).map((user) => {
          ...
          return '<option value="' + user.admin_name + '">' +
            roleText + ' - ' + user.admin_name +
          '</option>'
        }).join('')
    }
