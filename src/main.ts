  ...
  alert('서비스 목록 조회 실패: ' + serviceError.message)
          ...
          app.innerHTML = `
            <div class="pg-admin-page">
              ...
              <div class="merchant-toolbar">
                <button id="staff-go-admin">예약관리</button>
                <button id="staff-go-service">서비스관리</button>
                <button id="staff-go-staff">직원관리</button>
                <button id="staff-go-customer">고객관리</button>
              </div>
                ...
                <div class="merchant-product-layout">
                  ...
                  <div class="product-create-card">
                    ...
                    <div class="input-group">
                      ...
                      <div>
                        <label><input type="checkbox" class="beauty-staff-off-day" value="월" /> 월</label>
                        <label><input type="checkbox" class="beauty-staff-off-day" value="화" /> 화</label>
                        <label><input type="checkbox" class="beauty-staff-off-day" value="수" /> 수</label>
                        <label><input type="checkbox" class="beauty-staff-off-day" value="목" /> 목</label>
                        <label><input type="checkbox" class="beauty-staff-off-day" value="금" /> 금</label>
                        <label><input type="checkbox" class="beauty-staff-off-day" value="토" /> 토</label>
  ...
  <div class="beauty-service-check-list">
    ...
    `).join('')}
                  ...
                  <div class="product-list-card">
                    ...
                    <div class="product-summary-row">
                      <span>총 직원 : ${(staffList || []).length}명</span>
        
                      <span>
                        근무중 :
                        ${(staffList || []).filter(
                          (staff) => (staff.status || '근무중') === '근무중'
                        ).length}명
                      </span>
          ...
          ;(staffList || []).forEach((staff) => {
            ...
            const status = staff.status || '근무중'
        
            const offDays =
              Array.isArray(staff.off_days) && staff.off_days.length
                ? staff.off_days.join(', ')
                : '없음'
            ...
            item.innerHTML =
              ...
              '<div class="product-actions">' +
                '<button class="beauty-staff-edit" ' +
                  'data-id="' + staff.id + '">' +
                  '수정' +
                '</button>' +
        
                '<button class="beauty-staff-status" ' +
                  'data-id="' + staff.id + '" ' +
                  'data-status="' + status + '">' +
                  (status === '근무중'
                    ? '근무중지'
                ...
                '<button class="beauty-staff-delete" ' +
                  'data-id="' + staff.id + '">' +
                  '삭제' +
                '</button>' +
              '</div>'
        
            staffBody.appendChild(item)
          })
          ...
          document.querySelectorAll('.beauty-staff-edit')
            .forEach((button) => {
              button.addEventListener('click', async () => {
                const staffId =
                  Number(
                ...
                const newWorkStart =
                  prompt(
                    '근무 시작시간',
                    staff.work_start || '10:00'
                  )
        
                if (newWorkStart === null) return
        
                const newWorkEnd =
                  prompt(
                    '근무 종료시간',
                    staff.work_end || '19:00'
                  )
        
                if (newWorkEnd === null) return
          ...
          document.querySelectorAll('.beauty-staff-status')
            .forEach((button) => {
              button.addEventListener('click', async () => {
                const staffId =
                  Number(
          ...
          document.querySelectorAll('.beauty-staff-delete')
            .forEach((button) => {
              button.addEventListener('click', async () => {
                const staffId =
                  Number(
          ...
          document.querySelector('#staff-go-customer')
            ?.addEventListener('click', () => {
              alert('고객관리는 다음 단계에서 연결합니다.')
            })
