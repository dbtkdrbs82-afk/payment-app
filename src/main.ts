...
} else if (path === '/pg-admin-mobile-login') {

  const {
    renderPgAdminMobileLogin
  } = await import(
    './pg-admin-mobile-login'
  )

  await renderPgAdminMobileLogin(
    app
  )
  ...
  String(settlementCycle || '')
    .replace(/[^0-9]/g, ''),
          ...
          if (
            ...
            (
              !group.payout_time ||
              row.payout_time > group.payout_time
            )
          ) {
            group.payout_time = row.payout_time
          }
  ...
  ?.addEventListener(
    ...
    const openCashReceipt =
      (
        ...
        const cancelNumber =
          cancelData?.issueNumber ||
          cancelData?.approvalNumber ||
          receipt.cancel_receipt_key ||
          '-'
    ...
    document
      .querySelectorAll<HTMLButtonElement>(
        '.merchant-cash-approval-receipt'
      )
      .forEach(
        (
          button
